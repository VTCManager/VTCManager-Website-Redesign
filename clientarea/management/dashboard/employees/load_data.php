...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
		...
		if(strtotime($employee_time_online_raw) > strtotime("-2 minutes")) {
			if(strtotime($employee_last_client_update) > strtotime("-30 seconds")){
				...
				$employee_online_status = "<i class='fas fa-truck' style='color:green;'></i> online in der Nähe von ".$obj_trucky->response->poi->realName;
				}else{
