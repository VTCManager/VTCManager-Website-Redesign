...
$sql = "SELECT * FROM rank WHERE name='$rank' AND forCompanyID=$userCompanyID";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			// output data of each row
			while($row = $result->fetch_assoc()) {	
...
$myfile = fopen("../../../../media.northwestvideo.de/media/articles/ad_description/$old_num.txt", "w");
fwrite($myfile, $message);
fclose($myfile);

$conn->close();
header("Refresh:0; url=/clientarea/management/job_advertisements/");
			die();
