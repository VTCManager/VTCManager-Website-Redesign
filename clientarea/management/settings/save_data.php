...
if(isset($_POST["submit"])) {
    ...
    if($check !== false) {
		...
		$conn->close();
        header("Location: /clientarea/management/settings/?idc=pic_not_img");
	exit();
    }
}
// Check file size
if ($_FILES["fileToUpload"]["size"] > 5000000) {
	$conn->close();
    header("Location: /clientarea/management/settings/?idc=pic_too_lg");
	exit();
}
...
&& $imageFileType != "gif" ) {
	$conn->close();
    header("Location: /clientarea/management/settings/?idc=ic_format");
	exit();
}
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
		...
		$conn->close();
        header("Location: /clientarea/management/settings/?idc=server_fail");
	exit();
    }
}
